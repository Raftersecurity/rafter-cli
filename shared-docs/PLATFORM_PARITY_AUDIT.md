...
- **OpenClaw demoted from `--all`** — this PR (rf-0lig). Activity check confirms OpenClaw is highly active in 2026 (top GitHub repository, 4000+ ClawHub skills, regular releases). However, the current rafter integration shape — a markdown file dropped at `~/.openclaw/skills/rafter-security.md` — doesn't match ClawHub's actual skill format, so it's a wrong-shape no-op at the platform level. Decision: keep `--with-openclaw` as explicit opt-in (the install path is harmless if a user wants the file regardless), but **drop OpenClaw from `--all`** so we don't unconditionally ship a wrong-shape integration to users who didn't explicitly ask for it. Filed `rf-zgwj` (P2) for the proper ClawHub-skill rebuild.

rf-cia is now closed. The remaining matrix entries are all "yes" or upstream-limited.
