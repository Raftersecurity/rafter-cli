...
- **Continue.dev per-skill rules shipped** — this PR (rf-acz0). Adds 4 per-skill rule files at `.continue/rules/<skill>.md` with Continue.dev YAML frontmatter (`name:` + `description:` + `alwaysApply: false`). Continue.dev now also installs at `--local` scope. MCP entry is unchanged.
...
Outstanding rf-cia work after this PR: rf-044o (Gemini re-verify), rf-ovql (Codex schema), rf-65zg (`agent verify` enhancements), rf-o329 (onboarding doc), rf-0lig (OpenClaw activity check).
