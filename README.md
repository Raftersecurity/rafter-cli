...
To inject per-project agent guidance into your repo's instruction files (CLAUDE.md, AGENTS.md, etc.):

```sh
rafter agent init-project           # all supported platforms
rafter agent init-project --list    # preview what would be written
rafter agent init-project --only claude-code,codex  # specific platforms
```

This writes a Rafter security context block into project-level instruction files that agents read at session start. Commit these files so every agent session in the repo sees secure coding rules, scanning commands, and hook behavior. Use `--list` to preview without writing.
