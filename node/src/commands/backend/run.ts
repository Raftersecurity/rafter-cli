...
/**
 * Shared action handler for the remote backend scan.
 * Used by `rafter run`, `rafter scan` (default), and `rafter scan remote`.
 */
export async function runRemoteScan(opts: {
  ...
  if (!opts.quiet) {
    ...
    try {
      ...
      const exitCode = await handleScanStatus(data.scan_id, { "x-api-key": key }, format, opts.quiet);
...
/** Options shared by all commands that trigger a remote scan. */
function addRemoteScanOptions(cmd: Command): Command {
  return cmd
...
export function createRunCommand(): Command {
  return addRemoteScanOptions(
    new Command("run").description("Trigger a remote security scan")
