...
const RAFTER_MARKER_START = "<!-- rafter:start -->";
const RAFTER_MARKER_END = "<!-- rafter:end -->";

/** Rafter security context block for agent instruction files (CLAUDE.md, AGENTS.md, etc.) */
const RAFTER_INSTRUCTION_BLOCK = `${RAFTER_MARKER_START}
## Security: Rafter
...
/**
 * Write a Rafter instruction block into a global instruction file.
 * Uses marker comments for idempotent updates — existing rafter blocks are replaced,
 * non-rafter content is preserved.
 */
function injectInstructionFile(filePath: string): boolean {
