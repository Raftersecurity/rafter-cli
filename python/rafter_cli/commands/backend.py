...
def _do_remote_scan(
    ...
    resolved_github_token = github_token or _os.environ.get("RAFTER_GITHUB_TOKEN") or None
    try:
    ...
    if resolved_github_token:
        body["github_token"] = resolved_github_token
...
def register_backend_commands(app: typer.Typer) -> None:
    ...
    def run(
        ...
        github_token: str = typer.Option(None, "--github-token", "-g", envvar="RAFTER_GITHUB_TOKEN", help="GitHub token"),
        ...
        _do_remote_scan(repo, branch, api_key, fmt, skip_interactive, quiet, mode, github_token)
