...
- **A heredoc body is data, not a command** (sable-5ogx, [#230](https://github.com/Raftersecurity/rafter-cli/issues/230)). `cat > README.md <<'EOF' … EOF` writes its body to a file; it does not run it. Neither sanitizer had any heredoc handling, so the body reached the risk patterns as bare words and documentation that quoted a dangerous command classified `critical` — the unconditional hard block — making it impossible to write that file through the gate. Heredoc bodies are now consumed during tokenization and treated as data, *unless* something actually executes them: `bash <<EOF`, `ssh host <<EOF`, `cat <<EOF | bash` and `bash -c "$(cat <<EOF … EOF)"` all still hard-block. `<<-` tab-stripping, quoted delimiters, multiple heredocs per line, unterminated bodies and `<<<` here-strings are each handled explicitly. Full contract in `shared-docs/CLI_SPEC.md`.
- **[SECURITY] `bash -c "$(…)"` no longer bypasses the hard block** (sable-c6an). For a word that is entirely a command substitution the tokenizer leaves the literal text empty, and the `-c` branch read only that text — so the whole script argument vanished before matching and `bash -c "$(echo rm -rf /)"` classified `low`. So did `` sh -c "`echo rm -rf /`" `` and `echo "rm -rf /" | bash`. Two tokens, no policy file, no unusual environment: an evasion of the property documented as impossible to opt out of. The classifier now separates *this segment runs a string it was handed* from *this segment's output becomes code elsewhere*; `bash -c "echo 'rm -rf /'"` still only prints, and is still `low`.
